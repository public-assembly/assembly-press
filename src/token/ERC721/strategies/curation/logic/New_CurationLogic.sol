...
/**
* @title New_CurationLogic
* @notice New_CurationLogic for AssemblyPress architecture
*
* @author Max Bochman
* @author Salief Lewis
*/
contract New_CurationLogic is IERC721PressLogic, ICurationLogic, CurationStorageV1 { 
    ...
    function updateLogicWithData(address updateSender, bytes memory logicData) external {
        ...
        encodedListingsMapping[msg.sender][0] = logicData;
    }    

    event ListingDecoded(Listing decodedListing);

    function decodeListing(bytes memory _encodedListing) public returns (Listing memory) {
        (
            ...
            bool hasTokenId
        ) = abi.decode(_encodedListing, (address, uint96, address, int32, uint16, uint16, bool));

        Listing memory decodedListing = Listing(
            curatedAddress,
            selectedTokenId,
            curator,
            sortOrder,
        ...
        // event emitted for debugging
        emit ListingDecoded(decodedListing);

        return decodedListing;
    }  

    event ListingInput(bytes listingInput);

    function getTheListing(address target, uint _index) public returns (
        address curatedAddress,
        uint96 selectedTokenId,
        address curator,
        int32 sortOrder,
        ...
        // emitting for debugging purposes
        emit ListingInput(encodedListing);
        ...
        ( 
            ...
            uint16 curationType,
            bool hasId
        ) = abi.decode(encodedListing, (address, uint96, address, int32, uint16, uint16, bool));
