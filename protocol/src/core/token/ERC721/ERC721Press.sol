    ...
    IERC5192
{
    ...
    function _mintChecks(address sender, uint256 quantity, uint256 msgValue) internal {
        ...
        (bool success) =
            TransferUtils.safeSendETH(storageFeeRecipient, msgValue, TransferUtils._FUNDS_SEND_NORMAL_GAS_LIMIT);
    ...
    function _mintWithData(address sender, uint256 quantity, bytes calldata data) internal returns (uint256) {
        ...
        emit IERC721Press.MintWithData({sender: sender, quantity: quantity, firstMintedTokenId: firstMintedTokenId});
    ...
    function tokenURI(uint256 tokenId)
        public
        view
        override(ERC721AUpgradeable, IERC721Press)
     ...
     */
    function locked(uint256 tokenId) external view virtual override(IERC5192) returns (bool transferable) {
        if (_settings.transferable == true) {
            return false;
        } else {
            return true;
