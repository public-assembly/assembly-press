    ...
    IERC5192
{
     ...
     */
    function initialize(address initialOwner, address database, bytes memory init)
        external
        nonReentrant
        initializerERC721A
        initializer
    {
        // Decode init data
        (string memory name, string memory symbol) = abi.decode(init, (string, string));
        // Initialize ERC721A
     ...
     */
    function mint(address recipient, uint256 quantity) external onlyDatabase {
     ...
     */
    function burn(uint256 tokenId) external onlyDatabase {
     ...
     */
    function burnBatch(uint256[] memory tokenIds) external onlyDatabase {
