...
contract CurationDatabaseV1 is ERC721PressDatabaseSkeletonV1, ICurationTypesV1, DualOwnable {
    ...
    event FeeModuleSet(address indexed targetPress, address indexed feeModule);

    /// @notice Data has been sorted
    event DataSorted(address indexed targetPress, address indexed sortCaller, uint256[] ids, int128[] sortOrders);
    ...
    function getStorageFee(address targetPress, address storeCaller, uint256 storageSlots)
        external
        view
        returns (address, uint256)
    {
        return
            IERC721PressFeeModule(feeModuleInfo[targetPress]).getFeeInstructions(targetPress, storeCaller, storageSlots);
    }
    ...
    function storeData(address storeCaller, bytes calldata data) external requireInitialized(msg.sender) {
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            ...
            idToData[sender][storedCounter] = SSTORE2.write(tokens[i]);
            emit DataStored(sender, storeCaller, storedCounter + 1, idToData[sender][storedCounter]);
     ...
     */
    function canSort(address targetPress, address sortCaller)
        public
        view
        requireInitialized(targetPress)
        returns (bool sortAccess)
    {
