...
contract RolesWith721GateImmutableMetadataNoFees is IERC721PressLogic, ICurationLogic {
    ...
    event Erc721GateUpdated(address targetPress, address sender, address erc721Gate);

    /// @notice Event emitted when isPaused status updated
    /// @param targetPress ERC721Press being targeted
    /// @param sender msg.sender
    /// @param isPaused isPaused true/false bool
    event IsPausedUpdated(address targetPress, address sender, bool isPaused);
    ...
    event IsTokenDataImmutable(address targetPress, address sender, bool tokenImmutability);

    /// @notice Event emitted when frozenAt status updated
    /// @param targetPress ERC721Press being targeted
    /// @param sender msg.sender
    /// @param frozenAt unix timestamp in seconds
    event FrozenAtUpdated(address targetPress, address sender, uint80 frozenAt);
    ...
    event RoleAssigned(address targetPress, address sender, address account, uint8 role);

    /// @notice Event emitted when a role is revoked from an account
    /// @param targetPress ERC721Press being targeted
    /// @param sender msg.sender
    /// @param account account being revoked
    event RoleRevoked(address targetPress, address sender, address account);
    ...
    function _isAdmin(address targetPress, address account) internal view returns (bool) {
    ...
    /// @param targetPress target Press
    /// @param account account to check
    function _isAdminOrManager(address targetPress, address account) internal view returns (bool) {
    ...
    function initializeWithData(address targetPress, bytes memory data) external {
        ...
        (address erc721Gate, bool isPaused, bool isTokenDataImmutable, RoleDetails[] memory initialRoles) =
    ...
    function assignRoles(address targetPress, RoleDetails[] memory roleDetails) external onlyAdmin(targetPress) {
    ...
    function revokeRoles(address targetPress, address[] memory accounts) external onlyAdmin(targetPress) {
        // Revoke roles from each account provided
        for (uint256 i; i < accounts.length; ++i) {
            // revoke role from account
            roleInfo[targetPress][accounts[i]] = NO_ROLE;

            emit RoleRevoked({targetPress: targetPress, sender: msg.sender, account: accounts[i]});
        }
    }
    ...
    function _setErc721Gate(address targetPress, address erc721Gate) internal {
        ...
        emit Erc721GateUpdated({targetPress: targetPress, sender: msg.sender, erc721Gate: erc721Gate});
    }
    ...
    function _setIsPaused(address targetPress, bool isPaused) internal {
        ...
        emit IsPausedUpdated({targetPress: targetPress, sender: msg.sender, isPaused: isPaused});
    }
    ...
    function _setFrozenAt(address targetPress, uint80 frozenAt) internal {
        ...
        emit FrozenAtUpdated({targetPress: targetPress, sender: msg.sender, frozenAt: frozenAt});
    }
    ...
    function getAccessLevel(address targetPress, address accountToGetAccessFor) external view returns (uint256) {
    ...
    function _getAccessLevel(address targetPress, address accountToGetAccessFor) internal view returns (uint256) {
