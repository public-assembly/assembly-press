...
contract RolesWith721GateImmutableMetadataNoFeesTest is ERC721PressConfig {
    function test_initialize() public setUpCurationStrategy {
        ...
        (address erc721Gate, uint80 frozenAt, bool isPaused, bool isTokenDataImmutable) =
        ...
        require(
            RolesWith721GateImmutableMetadataNoFees(pressLogic).roleInfo(
                address(targetPressProxy), PRESS_ADMIN_AND_OWNER
            ) == ADMIN_ROLE
        );
        require(
            RolesWith721GateImmutableMetadataNoFees(pressLogic).roleInfo(address(targetPressProxy), PRESS_MANAGER)
                == MANAGER_ROLE
        );
        ...
        require(
            RolesWith721GateImmutableMetadataNoFees(pressLogic).roleInfo(address(targetPressProxy), PRESS_USER)
                == NO_ROLE
        );
        require(
            RolesWith721GateImmutableMetadataNoFees(pressLogic).roleInfo(address(targetPressProxy), PRESS_NO_ROLE_1)
                == NO_ROLE
        );
        require(
            RolesWith721GateImmutableMetadataNoFees(pressLogic).roleInfo(address(targetPressProxy), PRESS_NO_ROLE_2)
                == NO_ROLE
        );
    }
    ...
    function test_getMintPrice() public setUpCurationStrategy {
        (uint256 storedcounter, address logic, uint8 initialized, address renderer) =
            CurationDatabaseV1(address(targetPressProxy.getDatabase())).settingsInfo(address(targetPressProxy));
        (uint256 logicSpecific_UniversalMintPrice) =
            RolesWith721GateImmutableMetadataNoFees(logic).getMintPrice(address(targetPressProxy), PRESS_USER, 1);
    ...
    function test_GrantAndRevokeRoles() public setUpCurationStrategy {
        (uint256 storedcounter, address pressLogic, uint8 initialized, address renderer) =
            CurationDatabaseV1(address(targetPressProxy.getDatabase())).settingsInfo(address(targetPressProxy));
    ...
    function test_frozenAt() public setUpCurationStrategy {
        ...
        RolesWith721GateImmutableMetadataNoFees(pressLogic).setFrozenAt(
            address(targetPressProxy), (uint80(block.timestamp))
        );
    ...
    function test_erc721Gate() public setUpCurationStrategy {
        ...
        require(
            RolesWith721GateImmutableMetadataNoFees(pressLogic).getAccessLevel(address(targetPressProxy), PRESS_USER)
                == USER,
        ...
        vm.startPrank(PRESS_ADMIN_AND_OWNER);
        RolesWith721GateImmutableMetadataNoFees(pressLogic).setErc721Gate(
            address(targetPressProxy), address(mockAccessPass_2)
        );
        (address erc721Gate_again, uint80 frozenAt_again, bool isPaused_again, bool isTokenDataImmutable_again) =
            RolesWith721GateImmutableMetadataNoFees(pressLogic).settingsInfo(address(targetPressProxy));
