...
contract RolesWith721GateImmutableMetadataNoFeesTest is ERC721PressConfig {
    ...
    function test_GrantAndRevokeRoles() public setUpCurationStrategy {        
        ...
        require(RolesWith721GateImmutableMetadataNoFees(logic).roleInfo(address(targetPressProxy), PRESS_MANAGER) == MANAGER_ROLE, "incorrect role assigned");
        vm.stopPrank();
        ...
        RolesWith721GateImmutableMetadataNoFees(logic).assignRoles(address(targetPressProxy), newRoles);
        require(RolesWith721GateImmutableMetadataNoFees(logic).roleInfo(address(targetPressProxy), PRESS_MANAGER) == ADMIN_ROLE, "incorrect role assigned");        
        vm.stopPrank();
        ...
        RolesWith721GateImmutableMetadataNoFees(logic).revokeRoles(address(targetPressProxy), accountsToRevoke);
        require(RolesWith721GateImmutableMetadataNoFees(logic).roleInfo(address(targetPressProxy), PRESS_ADMIN_AND_OWNER) == NO_ROLE, "revoke did not work");                
    ...
    function test_frozenAt() public setUpCurationStrategy {
        ...
        RolesWith721GateImmutableMetadataNoFees(logic).setFrozenAt(address(targetPressProxy), (uint80(block.timestamp)));
    ...
    function test_erc721Gate() public setUpCurationStrategy {
        ...
        require(RolesWith721GateImmutableMetadataNoFees(logic).roleInfo(address(targetPressProxy), PRESS_USER) == NO_ROLE, "incorrect role assigned");         
        // check that PRESS_USER still has USER access due to having balance 1 of erc721Gate
        require(RolesWith721GateImmutableMetadataNoFees(logic).getAccessLevel(address(targetPressProxy), PRESS_USER) == USER, "incorrect access level");         
        // set up data for mint
        PartialListing[] memory listings = new PartialListing[](1);
        listings[0].chainId = 1;       
        listings[0].tokenId = 3;      
        ...
        require(RolesWith721GateImmutableMetadataNoFees(logic).getAccessLevel(address(targetPressProxy), PRESS_USER) == NO_ROLE, "incorrect access level");         
        ...
        vm.startPrank(PRESS_ADMIN_AND_OWNER);
        RolesWith721GateImmutableMetadataNoFees(logic).setErc721Gate(address(targetPressProxy), address(mockAccessPass_2));
        (
