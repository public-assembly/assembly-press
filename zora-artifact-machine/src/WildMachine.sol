...
/**
 * @title ArtifactMachine
 * @notice Initializes tokenURIs via WildInterface.updateArtifact
    ...
    ReentrancyGuard 
{
    ...
    /// @notice Artifact metadata update failed
    error MetadataUpdateFail();    
    ...
    address public artifactMachineMetadataRenderer;

    // ||||||||||||||||||||||||||||||||
    // ||| CONSTRUCTOR ||||||||||||||||
    // ||||||||||||||||||||||||||||||||  

    constructor(address _artifactMachineMetadataRenderer) {
    ...
    function createArtifact(
        ...
        _createArtifact(zoraDrop, mintRecipient, addressToCheck, tokenURIs);
    ...
    function _createArtifact(
        ...
        // for length of tokenURIs array, emit Mint event and updateArtifact
        for (uint256 i = 0; i < tokenUriArrayLength; i++) {
            ...
            (bool artifactSuccess) = WildInterface(artifactMachineMetadataRenderer).updateArtifact(
                zoraDrop,
                specificTokenId,
                addressToCheck,
                tokenURIs[i]
    ...
    /// @dev updates artifactMachineMetadataRenderer variable
    /// @param newRenderer new artifactMachineMetadataRenderer address
    function setMetadataRenderer(address newRenderer) public onlyOwner {

        artifactMachineMetadataRenderer = newRenderer;
